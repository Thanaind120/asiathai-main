...
class RoomController extends Controller
{
    public function index($id){
        ...
        $list2=Room::where('poolvilla_id',$id)->whereBetween('status',[0,1])->get();
        $data=array(
            'list1'=>$list1,
            'list2'=>$list2,
        );
        return view('Partner.Room.index',$data);
    }
    public function delete(Request $request){
        $list2=Room::where('id',$request->id)->first();
        $list2->status=3;
        $list2->save();
        return redirect()->to('Partner/Poolvilla/'.$list2->poolvilla_id.'/Rooms')->with('success','Delete Room success');
    }
    public function form1($id){
        $partner_id=Auth::guard('partner')->user()->id;
        // $list1=Poolvilla::where('partner_id',$partner_id)->get();
        $list2=Province::get();
        $list3=Poolvilla::where('id',$id)->first();
        $data=array(
            'list3'=>$list3,
            'list2'=>$list2,
            // 'list1'=>$list1,
        );
        return view('Partner.Room.form1',$data);
    }
    public function edit1($id){
        ...
        $list3=Poolvilla::where('id',$list1->poolvilla_id)->first();
        // dd($list3);
        $list2=District::get();
        $data=array(
            'list3'=>$list3,
    ...
    public function save_form1(Request $request){
        ...
        if($request->type==1){
            ...
            $room->name_th=$request->name_th;
            $room->poolvilla_id=$request->poolvilla_id;
            $room->price=$request->price;
            $room->twin_bed=$request->twin_bed;
            $room->full_bed=$request->full_bed;
            $room->queen_bed=$request->queen_bed;
            $room->king_bed=$request->king_bed;
            ...
            $room->max_room=$request->max_room;        
            $room->save();
            IconRoom::where('room_id',$request->id)->delete();
            foreach($request->icon as $i){
            $icon=new IconRoom();
            $icon->room_id=$room->id;
            $icon->icon_id=$i;
        ...
        return redirect()->to('Partner/Room/'.$room->id.'/Images');
    ...
    public function delete_image_room($id,$image_id){
    
               $image_pool=ImageRoom::where('id',$image_id)->first();
               unlink($image_pool->image);
               ImageRoom::where('id',$image_id)->delete();
          
    
            return redirect()->to('Partner/Room/'.$id.'/Images');
        }
